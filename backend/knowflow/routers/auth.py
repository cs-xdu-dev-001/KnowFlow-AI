...
@router.post("/api/auth/login", tags=["认证授权"], summary="本地账号登录")
def login(payload: LoginIn, request: Request, response: Response) -> dict[str, Any]:
    ...
    if not user:
        raise_api_error(401, 40102, "账号不存在，请先注册或检查输入是否正确")
    if not user.get("password_hash"):
        provider = user.get("auth_provider") or "第三方"
        provider_name = {"github": "GitHub"}.get(str(provider).lower(), provider)
        raise_api_error(401, 40104, f"该账号通过 {provider_name} 创建，请使用对应的授权登录")
    if not verify_password(payload.password, user.get("password_hash")):
        raise_api_error(401, 40103, "密码不正确，请重新输入")
    create_auth_session(response, int(user["id"]), request)
    return api_success({"user": normalize_user(user)})


@router.post("/api/auth/logout", tags=["认证授权"], summary="退出登录")
...
@router.get("/api/auth/oauth/github/start", tags=["认证授权"], summary="GitHub OAuth 登录入口")
def github_oauth_start() -> RedirectResponse:
    if not GITHUB_CLIENT_ID or not GITHUB_CLIENT_SECRET:
        raise_api_error(400, 40020, "GitHub OAuth 尚未配置")
    query = urlencode(
        {
...
@router.get("/api/auth/oauth/github/callback", tags=["认证授权"], summary="GitHub OAuth 回调")
def github_oauth_callback(code: str, state: str, request: Request) -> RedirectResponse:
    if not GITHUB_CLIENT_ID or not GITHUB_CLIENT_SECRET:
        raise_api_error(400, 40020, "GitHub OAuth 尚未配置")
    if not verify_oauth_state(state):
        raise_api_error(400, 40021, "GitHub 授权状态校验失败，请重新发起登录")
    ...
    if not access_token:
        raise_api_error(400, 40022, "GitHub 授权失败，未获取到访问令牌")
    ...
    if not provider_user_id:
        raise_api_error(400, 40023, "GitHub 用户信息异常，请重新授权")
